...
classDiagram
    ...
    App --> NewsPage : routes to
    App --> ReportPage : routes to
    App --> AdminDashboard : routes to
    NewsPage ..> SanityClient : uses
    ReportPage ..> SanityClient : uses
```
...
sequenceDiagram
    ...
    participant SanityClient as Sanity API
    
    User->>Frontend: Mengisi form laporan
    User->>Frontend: Upload file bukti (Drag & Drop)
    User->>Frontend: Klik Submit
    Frontend->>Frontend: Validasi Form
    Frontend->>SanityClient: Upload Asset (File)
    SanityClient-->>Frontend: Return Asset ID
    Frontend->>SanityClient: Create Document (Report + Asset ID)
    SanityClient-->>Frontend: Return Success Status
...
sequenceDiagram
    actor Masyarakat
    participant Website as Halaman Informasi
    participant SanityClient as Sanity API
    
    Masyarakat->>Website: Mengetik kata kunci pencarian
    Website->>Website: Menangkap input 'searchQuery'
    Website->>Website: Memfilter daftar 'infoList' yang sudah ada
    Website-->>Masyarakat: Menampilkan hasil pencarian secara real-time
```
